...
fn day4_part1(cards: &[Card]) -> usize {
	cards.iter().map(Card::count).map(|s| if s > 0 {
		2_usize.pow((s - 1) as _)
	} else {
		0
	}).sum()
}
...
fn day4_part2(cards: &[Card]) -> usize {
	...
	let mut scores: Vec<G> = cards.iter().map(|c| G { count: 1, score: c.count() }).collect();

	for i in 0..scores.len() {
		let upper_bound = if i + scores[i].score > scores.len() { scores.len() } else { i + scores[i].score };
...
mod tests {
	...
	fn test_parse_set() {
		...
		assert_eq!(card_parser::set(input), Ok(vec![83, 86, 6, 31, 17, 9, 48, 53]))
	}
	...
	fn test_parse_card() {
		...
		assert_eq!(card_parser::card(input), Ok(Card {
			id: 1,
